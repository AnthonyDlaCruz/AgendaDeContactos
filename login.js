...
function credenciales(email, pass) {
  const usuario1 = ["anthony@gmail.com", "abcd"];
  const usuario2 = ["kessiae@gmail.com", "dfgh"];

  if (
    (email == usuario1[0]) & (pass == usuario1[1]) ||
    (email == usuario2[0]) & (pass == usuario2[1])
  ) {
    swal.fire({
      icon: "success",
...
function bloqueados(email, pass) {
  if (email == "kessiae@gmail.com" || email == "mabel09@gmail.com") {
    swal.fire({
      icon: "warning",
